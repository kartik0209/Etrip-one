...
const Sconf = () => {
    ...
    const hbook = async () => {
        ...
        let result = await fetch("/hbook", {
            ...
            body: JSON.stringify({ hotel: rooms.Hname, troom: "super", hotelid: rooms._id, userid: "11", Fromdate: fdate, todate: tdate, totalamount: rooms.SRoomprice * totaldays + tax, totalday: totaldays, payid: 1, status: "booked" })
    ...
    return (<>
        {loading ? (<h1>loading</h1>) : error ? (<h2>eroor</h2>) : (<><h1>rooms</h1>
            ...
            {(cupon == "E-TRIP10") ? (<>
                ...
                <h2>totalamount--{(rooms.SRoomprice * totaldays) + tax - 2000} </h2></>) 
                :
                 (<><h2>totalamount--{(rooms.SRoomprice * totaldays) + tax} </h2></>)}

            <h2><img src={rooms.SRoomurl} alt="" height="200px" /></h2>

            <button onClick={hbook}>paynow</button>
        </>)
